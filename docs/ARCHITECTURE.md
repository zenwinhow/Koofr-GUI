...
├── work_directory.rs           工作目录定位与启动期迁移
...
默认工作目录是 Tauri `app_local_data_dir`。用户可以在设置中选择另一个已有的空目录，
并决定是否把当前工作目录中的设置、传输检查点、下载历史、缓存、日志以及其他文件全量
迁移过去。变更不会在运行中搬动已打开的文件，而是写入默认数据目录旁的
`net.koofr.desktop.gui.work-directory.json` 定位记录，在下次启动、构造 `AppState` 前执行。

全量迁移先递归复制到目标同级的应用专用暂存目录，拒绝符号链接和特殊文件；写入迁移
标记后再激活目标，最后清理旧目录。这样既支持跨卷移动，也能在进程或 I/O 中断后区分
源目录、完整目标和暂存副本。只有目标激活后才清理源文件；失败时保留可用副本并在下次
启动重试。目标必须为空且不是文件系统根目录，新旧目录不能相同或互相嵌套。

该定位记录是找到自定义目录所必需的引导数据，不属于可迁移工作数据；默认工作目录
作为源时它会留在原位。Windows Credential Manager 中的应用专用密码也不属于
工作目录，既不会经 IPC 暴露，也不会被迁移。
...
| `Disk` | 当前工作目录的 `cache/metadata-cache.json` | 可配置 |

- 缓存 key = `(userId, mountId, remotePath)`，按账户完全隔离。
- 缓存 value = **仅**文件名 + 远程路径 + 大小 + 修改时间。**不缓存令牌、密码、文件内容**。
- 切换模式、清缓存、切换账户时自动失效。
- 磁盘缓存随整个工作目录迁移，不再单独配置路径。

### 诊断日志

`AppLogger` 使用独立后台线程写入当前工作目录的 `logs/`，不阻塞 Tauri 命令或传输任务。设置页可以指定记录级别、保留天数和单文件大小上限，也可以查看占用并一键清理。
...
设置：`get_settings`, `update_settings`, `update_download_settings`, `update_logging_settings`, `update_transfer_settings`, `update_work_directory`, `clear_metadata_cache`, `clear_logs`, `select_work_directory`
...
├─ cache/metadata-cache.json    # 磁盘缓存
└─ logs/koofr-gui*.jsonl       # 诊断日志
```

默认位置旁另保留 `%LOCALAPPDATA%\net.koofr.desktop.gui.work-directory.json` 定位
记录；使用自定义工作目录时该文件仍留在原位。
